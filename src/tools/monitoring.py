...
from ..utils.api_client import AAPAPIError, aap_get, format_job_status, paginate_params
...
def register(mcp: FastMCP):
    ...
    class GetJobOutputInput(BaseModel):
        ...
        format: str = Field(default="txt", description="Output format: 'txt' (human-readable) or 'json' (structured)")
        max_lines: int = Field(default=200, ge=1, le=5000, description="Maximum number of output lines to return")
    ...
    class GetFailedJobsInput(BaseModel):
        ...
        job_template: str | None = Field(default=None, description="Filter by job template name")
    ...
    async def aap_get_failed_jobs(params: GetFailedJobsInput, ctx: Context) -> str:
        ...
        try:
            ...
            since = (datetime.now(UTC) - timedelta(hours=params.hours)).isoformat()
            q = {
                "status": "failed",
                "finished__gte": since,
    ...
    class GetActivityInput(BaseModel):
        ...
        operation: str | None = Field(
            default=None, description="Filter by operation: 'create', 'update', 'delete', 'associate'"
        )
        object_type: str | None = Field(
            default=None, description="Filter by object type: 'job_template', 'inventory', 'project', etc."
        )
        username: str | None = Field(default=None, description="Filter by AAP username")
